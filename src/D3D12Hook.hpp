...
public:
  ...
  typedef std::function<void(D3D12Hook&)> OnCreateSwapChainFn;
    ...
    inline void on_present(OnPresentFn fn) {
        m_on_present = fn;
    }

    inline void on_post_present(OnPresentFn fn) {
        m_on_post_present = fn;
    }

    inline void on_resize_buffers(OnResizeBuffersFn fn) {
        m_on_resize_buffers = fn;
    }

    inline void on_resize_target(OnResizeTargetFn fn) {
...
//    }
    ...
    inline ID3D12Device4* get_device() const {
        return m_device;
    }

    inline IDXGISwapChain3* get_swap_chain() const {
        return m_swap_chain;
    }
    ...
    inline ID3D12CommandQueue* get_command_queue() const {
        return m_command_queue;
    }

    inline UINT get_display_width() const {
        return m_display_width;
    }

    inline UINT get_display_height() const {
        return m_display_height;
    }

    inline UINT get_render_width() const {
        return m_render_width;
    }

    inline UINT get_render_height() const {
        return m_render_height;
    }

    inline bool is_inside_present() const {
        return m_inside_present;
    }

    inline bool is_proton_swapchain() const {
        return m_using_proton_swapchain;
    }

    inline void ignore_next_present() {
        m_ignore_next_present = true;
    }

    inline void set_next_present_interval(uint32_t interval) {
...
protected:
    ...
    static void STDMETHODCALLTYPE set_render_targets(ID3D12GraphicsCommandList* cmd_list, UINT NumRenderTargetDescriptors, const D3D12_CPU_DESCRIPTOR_HANDLE* pRenderTargetDescriptors, BOOL RTsSingleHandleToDescriptorRange, D3D12_CPU_DESCRIPTOR_HANDLE* depth_stencil_descriptor);
    static void STDMETHODCALLTYPE create_depth_stencil_view(ID3D12Device* device, ID3D12Resource* pResource, const D3D12_DEPTH_STENCIL_VIEW_DESC* pDesc, D3D12_CPU_DESCRIPTOR_HANDLE DestDescriptor);
    static HRESULT STDMETHODCALLTYPE create_committed_resource(ID3D12Device* device, const D3D12_HEAP_PROPERTIES* pHeapProperties, D3D12_HEAP_FLAGS HeapFlags, const D3D12_RESOURCE_DESC* pDesc, D3D12_RESOURCE_STATES InitialResourceState, const D3D12_CLEAR_VALUE* pOptimizedClearValue, REFIID riid, void** ppvResource);
