...
struct EmulatedView {
    ...
    uint32_t stereoPassMask{0x1}; // 0x1=Primary, 0x2=Secondary
};
...
public:
    ...
    const EmulatedView& getView(ViewType t) const { 
        return m_views[static_cast<size_t>(t)]; 
    }
...
private:
    StereoEmulator() = default;
    ~StereoEmulator() = default;
    
    StereoEmulator(const StereoEmulator&) = delete;
    StereoEmulator& operator=(const StereoEmulator&) = delete;
    ...
    float m_fovealFov{40.0f};
    float m_peripheralFov{110.0f};
    float m_fovealScale{1.0f};
