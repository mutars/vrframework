...
#include "core/VRFramework.hpp"
#include "core/IGameAdapter.hpp"
#include "template/games/godofwar/GowAdapter.hpp"
...
std::unique_ptr<VRFramework> g_framework{};

void VRFramework::hook_monitor() {
    ...
    if (d3d11 == nullptr || d3d12 == nullptr
        || (renderer_type == VRFramework::RendererType::D3D11 && d3d11 != nullptr && !d3d11->is_inside_present())
        || (renderer_type == VRFramework::RendererType::D3D12 && d3d12 != nullptr && !d3d12->is_inside_present()))
    {
...
VRFramework::VRFramework(HMODULE module)
    : m_module{module}
    , m_game_module{GetModuleHandle(0)}
    {
    m_game_adapter = std::make_unique<GowAdapter>();
...
void VRFramework::run_imgui_frame(bool from_present) {
...
void VRFramework::on_frame_d3d11() {
    std::scoped_lock _{ m_imgui_mtx };
    ...
    if (is_init_ok) {
        ...
        if (!std::exchange(m_created_default_cfg, true)) {
            if (!fs::exists({utility::widen(get_persistent_dir(m_game_adapter->getConfigFileName()).string())})) {
                save_config();
            }
        }
    }
...
void VRFramework::on_post_present_d3d11() {
...
// D3D12 Draw funciton
void VRFramework::on_frame_d3d12() {
    std::scoped_lock _{ m_imgui_mtx };
...
#if defined(_DEBUG) && defined(VRFRAMEWORK_EXPERIMENTAL)
...
#endif
#ifdef defined(_DEBUG) && defined(VRFRAMEWORK_EXPERIMENTAL)
...
void VRFramework::on_post_present_d3d12() {
    if (!m_error.empty() || !m_initialized || !m_game_data_initialized) {
        if (m_last_present_time <= std::chrono::steady_clock::now()){
            m_last_present_time = std::chrono::steady_clock::now();
        }
...
void VRFramework::patch_set_cursor_pos() {
...
void VRFramework::remove_set_cursor_pos_patch() {
    std::scoped_lock _{ m_patch_mtx };
...
bool VRFramework::on_clip_cursor(LPRECT* lpRect) {
    if(m_draw_ui) {
        *lpRect = nullptr;
        return true;
    }
...
bool VRFramework::on_message(HWND wnd, UINT message, WPARAM w_param, LPARAM l_param) {
    m_game_adapter->onMessage(wnd, message, w_param, l_param);
    m_last_message_time = std::chrono::steady_clock::now();
...
// this is unfortunate.
void VRFramework::on_direct_input_keys(const std::array<uint8_t, 256>& keys) {
...
std::filesystem::path VRFramework::get_persistent_dir() {
...
void VRFramework::save_config() {
    std::scoped_lock _{m_config_mtx};

    m_wants_save_config = false;

    spdlog::info("Saving config {}", m_game_adapter->getConfigFileName());
    ...
    try {
        if (!cfg.save((get_persistent_dir() / m_game_adapter->getConfigFileName()).string())) {
...
void VRFramework::set_draw_ui(bool state, bool should_save) {
    std::scoped_lock _{m_config_mtx};
...
void VRFramework::consume_input() {
...
int VRFramework::add_font(const std::filesystem::path& filepath, int size, const std::vector<ImWchar>& ranges) {
    ...
    m_additional_fonts.emplace_back(VRFramework::AdditionalFont{filepath, size, ranges, (ImFont*)nullptr});
    m_fonts_need_updating = true;

    return m_additional_fonts.size() - 1;
}

void VRFramework::update_fonts() {
...
void VRFramework::invalidate_device_objects() {
...
void VRFramework::draw_ui() {
    ...
    bool is_open = true;
    ImGui::Begin((m_game_adapter->getGameKey() + " VR").c_str(), &is_open);
...
//    draw_about();
    ...
    if (m_error.empty() && m_game_data_initialized) {
        ...
        ImGui::TextWrapped("%s is currently initializing...", m_game_adapter->getGameKey());
        ImGui::TextWrapped("This menu will close after initialization if you have the remember option enabled.");
    } else if (!m_error.empty()) {
        ImGui::TextWrapped("%s error: %s", m_game_adapter->getGameKey(), m_error.c_str());
    }
...
void VRFramework::set_imgui_style() noexcept {
...
//        ImNodes::CreateContext();
        ...
        static const auto imgui_ini = (get_persistent_dir() / m_game_adapter->getImguiIniName()).string();
...
bool VRFramework::initialize_game_data() {
    ...
    std::thread init_thread([this]() {
        ...
        try {
            m_game_adapter->preInit(*this);
            //gowvr::initialize_sdk();


            m_mods = std::make_unique<Mods>(m_game_adapter->createMods());
...
bool VRFramework::initialize_xinput_hook() {
...
void VRFramework::post_message(UINT message, WPARAM w_param, LPARAM l_param) {
    if (m_wnd == nullptr) {
        return;
    }
...
bool VRFramework::initialize_windows_message_hook() {
    if (m_wnd == 0) {
        return false;
    }
...
bool VRFramework::first_frame_initialize() {
    const bool is_init_ok = m_error.empty() && m_game_data_initialized;
...
void VRFramework::call_on_frame() {
    const bool is_init_ok = m_error.empty() && m_game_data_initialized;
